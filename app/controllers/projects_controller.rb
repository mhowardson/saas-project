class ProjectsController < ApplicationController
  ...
  def users
    @project_users = (@project.users + (User.where(tenant_idL @tenant.id, is_admin: true))) - [current_user]
    @other_users = @tenant.where(tenant_id: @tenant.id, is_admin: false) - (@project_users + [current_user])
  end
  ...
  private
    ...
    def verify_tenant
      unless params[:tenant_id] == Tenant.current_tenant_id.to_s
        redirect_to :root, flash: { error: 'You are not authorized to access any other organizations outside your own'}
      end
    end
end
